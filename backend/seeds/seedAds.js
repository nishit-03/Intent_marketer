...
async function seed(skipConnect = false) {
    ...
    for (let d = 0; d < 14; d++) {
      ...
      await AdMetric.create({
        ...
        intent_stages: {
          ...
          comparing: Math.floor(clicks * 0.35),
          buying: Math.floor(clicks * 0.25),
        },
      });
    }
  }
  ...
  const sampleSessions = [
    { categories: ['tech'], pages: 5, intent: 0.65, stage: 'comparing' },
    { categories: ['finance'], pages: 3, intent: 0.40, stage: 'exploring' },
    { categories: ['travel', 'tech'], pages: 8, intent: 0.80, stage: 'buying' },
    { categories: ['health'], pages: 2, intent: 0.15, stage: 'browsing' },
    { categories: ['education', 'tech'], pages: 6, intent: 0.55, stage: 'comparing' },
  ];
  ...
  for (const sessData of sampleSessions) {
    const session = new Session({
      ...
      comparison_count: sessData.stage === 'comparing' ? 2 : 0,
      product_views: sessData.stage === 'buying' ? 3 : 0,
    });
    await session.save();
